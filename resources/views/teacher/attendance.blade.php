                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            @foreach($dates as $date)
                                <th class="p-3 {{ $date === $today ? 'today' : 'disabled' }}">{{ $date }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($students as $student)
                            <tr class="border-b">
                                ...
                                @foreach($dates as $date)
                                    <td class="p-3 {{ $date === $today ? 'today' : 'disabled' }}">
                                        <input type="hidden" name="attendances[{{ $student->id }}][{{ $date }}][student_id]" value="{{ $student->id }}">
                                        <input type="hidden" name="attendances[{{ $student->id }}][{{ $date }}][date]" value="{{ \Carbon\Carbon::createFromFormat('d.m.y', $date)->format('Y-m-d') }}">
                                        <input type="hidden" name="attendances[{{ $student->id }}][{{ $date }}][is_present]" value="0">
                                        <input class="border-gray-300 rounded" type="checkbox" name="attendances[{{ $student->id }}][{{ $date }}][is_present]" value="1"
                                            @if(isset($attendances[$date]) && $attendances[$date]->where('student_id', $student->id)->first()?->is_present)
                                                checked
                                            @endif
                                            @if($date !== $today)
                                                disabled
